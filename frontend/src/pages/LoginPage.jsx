...
function LoginPage({ onLogin, initialError = "" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!data?.session?.access_token) throw new Error("로그인 응답이 올바르지 않습니다.");
      localStorage.setItem("access_token", data.session.access_token);
      onLogin(email.split("@")[0]);
    } catch (err) {
      setError(err.message || "로그인 서버에 연결할 수 없습니다.");
    } finally {
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      setError(err.message || "소셜 로그인 서버에 연결할 수 없습니다.");
    } finally {
      setLoginLoading(false);
    }
  };
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      setSignupError(err.message || "회원가입 서버에 연결할 수 없습니다.");
      return;
    } finally {
      ...
      <div className={`login-panel ${showLogin ? "visible" : ""}`}>
        <div className="login-card-v2">
          ...
          <div className="social-buttons">
            <button className="social-btn google" disabled={loginLoading} onClick={() => handleSocialLogin('google')}>
            ...
            <button className="social-btn github" disabled={loginLoading} onClick={() => handleSocialLogin('github')}>
